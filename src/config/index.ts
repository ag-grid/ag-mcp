...
const detectFramework = async (
  projectPath: string
): Promise<"react" | "angular" | "vue" | "vanilla"> => {
  try {
    ...
    if (dependencies["ag-grid-vue"] || dependencies["ag-grid-vue3"])
      return "vue";
...
const detectAgGridVersion = async (
  projectPath: string
  ...
  try {
    ...
    let agGridVersion =
      dependencies["@ag-grid-community/core"] ||
...
export const readProject = async (
  ...
  const hasAgGrid =
    isPackageExists("ag-grid-community", { paths: [path] }) ||
