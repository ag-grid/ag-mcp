...
import { detectAndSetProject, getCurrentProject } from "../state/project.js";
import { Semver } from "../utils/semver.js";

const listing = {
  name: "migrate-version",
  description:
    "Help the user migrate to a more recent version of AG-Grid with framework-specific guidance",
  arguments: [
    {
      name: "version",
    ...
    },
    {
      name: "framework",
      description:
        "Framework (react, angular, vue, vanilla) - will auto-detect if not provided",
      required: false,
    },
    {
      name: "currentVersion",
...
async function handler(
  ...
  const currentVersion =
    args.currentVersion || currentProject?.version || "unknown";
  ...
  if (currentVersion !== "unknown") {
    try {
      ...
      migrationPath = Semver.generateMajorVersionPath(
        currentVersion,
        targetVersion
      );
...
- Consult the resource: \`ag-mcp://article/upgrading-to-ag-grid-${targetMajorVersion}\` for specific upgrade instructions
...
**Migration Path:** v${currentMajorVersion} → ${migrationPath
      .map((v) => `v${v}`)
      .join(" → ")}
...
2. **Follow the official upgrade guide:** \`ag-mcp://article/upgrading-to-ag-grid-${nextMajorVersion}\`
...
4. **Breaking Changes:**
   - Consult \`ag-mcp://article/upgrading-to-ag-grid-${nextMajorVersion}\` for all breaking changes
...
2. **Follow official upgrade guide:** \`ag-mcp://article/upgrading-to-ag-grid-${targetMajorVersion}\`
3. **Manual migration only** - use the specific changes documented in the upgrade resource

**Step-by-Step Migration Process:**

1. **Review Breaking Changes:**
   - Consult \`ag-mcp://article/upgrading-to-ag-grid-${targetMajorVersion}\` for complete list
...
3. **Code Updates (${framework}):**
   ...
   - ${frameworkInfo.additional}
   - Apply changes from \`ag-mcp://article/upgrading-to-ag-grid-${targetMajorVersion}\` resource
...
${
  framework === "react"
    ...
    : ""
      ...
      framework === "vanilla"
        ? "- DOM integration updates\n- Event handling changes\n- CSS/styling updates\n- Grid creation API"
        : ""
    }

**Next Steps:**
1. Consult \`ag-mcp://article/upgrading-to-ag-grid-${targetMajorVersion}\` for detailed changes
